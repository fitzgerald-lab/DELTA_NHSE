...
kf <- fread(
    "/Users/somers01/OneDrive - University of Cambridge/ECI_local/OptimalScreening/Git/DataCleaning/Keith+Greta.csv", 
    na.string = ""
)
kf <- kf[, 
    c(
...
GetTimeToEvent <- function(
    ...
    tibble::tibble(time_to_E0Sponge, time_to, event)
}
...
for(risk in unique(df$Sponge_risk_strat)){
    new_rows <- df %>% 
        ...
        arrange(time_to) %>%  
        group_by(time_to, DELTA_ID, CYT_ID, Age) %>%
...
GetRates <- function(df, primary=FALSE){
    ...
    prevalence <- round(n_outcomes/n_total*100, digits = 1)
    ci = prop.test(n_outcomes, n_total)$conf.int[1:2]
    ci_low <- round(ci[1]*100, digits = 1)
    ci_hi <- round(ci[2]*100, digits = 1)
    ...
    ci_low2 <- ci[1]*const*100
    ci_hi2 <- ci[2]*const*100
...
demographics <- data.frame(
    rbind(
        GetDemographics(df), 
        GetDemographics(df[df$Clin_risk_strat == "Low risk", ]), 
        GetDemographics(df[df$Clin_risk_strat == "Moderate risk", ])
    )
) %>% round(digits = 3)
...
# chi-squared test 
chisq_df <- data.frame(
  LGD = df$General_histopathology %in% c("LGD"), 
  Sponge_risk_strat = df$Sponge_risk_strat
)
chisq_table <- xtabs(~LGD + Sponge_risk_strat, data = chisq_df)
chisq.test(chisq_table, correct=FALSE)
# PPV's 
dysplasia <- c("Crypt dysplasia", "LGD", "OAC", "HGD", "IMC")
dysplasia <- c("HGD", "OAC", 'IMC')
RG <- df[df$Sponge_risk_strat == "Low risk", ]
...
lode_plot <- ggplot(
    ...
    geom_text(
        stat = "stratum", 
        aes(
            label = ifelse(round(100*after_stat(prop), digits=1) %in% c(61.5, 53.7) , paste0(round(100*after_stat(prop), digits =1)%>% format(nsmall=1), "%"), ""),
            angle=90
        ), 
        colour = "white",
        size = 5,
...
forest_df$pval <- riskratio(risk_table)$p.val[-1,2] # use Fisher's exact test 
